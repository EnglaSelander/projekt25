 ...
 *              to connect with. 
 * @param size - the number of people (nodes) in the graph 
...
export function split_group_if_too_big (group: number[], max_group_size: number): number[][]{
    ...
    return [...split_group_if_too_big(first_half, max_group_size), 
            ...split_group_if_too_big(second_half, max_group_size)]; 
}

/**
 * Groups the nodes of a given listgraph into connected components
 * using BFS - Breadth First Search. 
 * @param graph - A listgraph representing people and their connections. 
 * @returns An array of arrays where each inner array represents a 
...
export function merge_bfs_groups(people_groups: number[][]): number[][] {
    const groups: number[][] = []; 
    const visited = new Set<number>();

    for (const current_group of people_groups) {
        let is_merged = false;
...
/**
 * @param graph - A people_graph represneting people and there connections. 
 ...
 */
export function main_divide_into_groups(graph: ListGraph, num_groups: number): number[][] {
